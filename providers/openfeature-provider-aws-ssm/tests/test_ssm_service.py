...
class TestNormalizeParameterName:
    """Tests for _normalize_parameter_name method."""

    def test_normalize_parameter_name_without_leading_slash(self):
        """Test that parameter name without leading slash gets one prepended."""
        service = SsmService()
        result = service._normalize_parameter_name("my-flag")
        assert result == "/my-flag"

    def test_normalize_parameter_name_with_leading_slash(self):
        """Test that parameter name with leading slash is unchanged."""
        service = SsmService()
        result = service._normalize_parameter_name("/my-flag")
        assert result == "/my-flag"

    def test_normalize_parameter_name_nested_path(self):
        """Test normalization with nested paths."""
        service = SsmService()
        result = service._normalize_parameter_name("app/env/flag")
        assert result == "/app/env/flag"

    def test_normalize_parameter_name_nested_path_with_slash(self):
        """Test normalization with nested paths that already have leading slash."""
        service = SsmService()
        result = service._normalize_parameter_name("/app/env/flag")
        assert result == "/app/env/flag"
...
class TestGetParameterValue:
    ...
    @patch("boto3.client")
    def test_get_parameter_value_preserves_leading_slash(self, mock_boto_client):
        """Test that flags with leading slash are preserved."""
        mock_client = MagicMock()
        mock_boto_client.return_value = mock_client
        mock_client.get_parameter.return_value = {"Parameter": {"Value": "value"}}
