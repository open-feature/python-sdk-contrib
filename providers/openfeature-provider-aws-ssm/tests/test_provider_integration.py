...
@pytest.mark.parametrize(
    ("flag_key", "param_name", "param_value"),
    [
        ("app/prod/feature-flag", "/app/prod/feature-flag", "prod-value"),
        ("app/dev/feature-flag", "/app/dev/feature-flag", "dev-value"),
        ("service/api/rate-limit", "/service/api/rate-limit", "100"),
        ("config/database/url", "/config/database/url", "postgres://localhost"),
    ],
)
def test_flag_key_hierarchical_paths_without_leading_slash(
    provider, ssm_client, flag_key, param_name, param_value
):
    ssm_client.put_parameter(Name=param_name, Value=param_value, Type="String")
    result = provider.resolve_string_details(flag_key, "default", None)
    assert result.value == param_value
    assert result.reason == Reason.STATIC
